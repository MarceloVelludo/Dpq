...
class DinamicaPontosQuanticos:
    ...
    def __init__(self, j_1_inicial=0, j_1_final=1, passoJ_1 = 0.5,
        ...
        self.ro0 = cupy.array([[0.25,0.25,0.25,0.25], [0.25,0.25,0.25,0.25], [0.25,0.25,0.25,0.25],[0.25,0.25,0.25,0.25]])
        #Criando as matrizes de Pauli-X, Pauli-Y, e Pauli-Z.
        self.sigmaX = cupy.array([[0, 1], [1, 0]])
        self.sigmaY = cupy.array([[0, -1j], [1j, 0]])
        self.sigmaZ = cupy.array([[1, 0], [0, -1]])
        
        #Matriz identidade.
        self.ident = cupy.identity(2)
        
        #Algumas constantes que são usadas diversas vezes para os calculos.
        self.tensorProductIdentSigX = cupy.reshape(cupy.tensordot(self.ident, self.sigmaX, axes=0), (4,4))
        self.tensorProductSigXIdent = cupy.reshape(cupy.tensordot(self.sigmaX, self.ident, axes=0), (4,4), order='A')
        self.tensorProductIdentSigY = cupy.reshape(cupy.tensordot(self.ident, self.sigmaY, axes=0), (4,4))
        self.tensorProductSigYIdent = cupy.reshape(cupy.tensordot(self.sigmaY, self.ident, axes=0), (4,4))
        self.tensorProductIdentSigZ = cupy.reshape(cupy.tensordot(self.ident, self.sigmaZ, axes=0), (4,4))
        self.tensorProductSigZIdent = cupy.reshape(cupy.tensordot(self.sigmaZ, self.ident, axes=0), (4,4))
        self.tensorProductSigZIdentSoma = cupy.reshape(cupy.tensordot(self.sigmaZ, self.ident, axes=0), (4,4)) + cupy.reshape(cupy.tensordot(self.ident, self.sigmaZ, axes=0), (4,4))
        self.tensorProductSigZSigZ = cupy.reshape(cupy.tensordot(self.sigmaZ, self.sigmaZ, axes=0), (4,4))
        ...
        #Cupy - to device
        self.arrayJ_1 = cupy.array(self.arrayJ_1)
        self.arrayJ_2 = cupy.array(self.arrayJ_2)
        self.arrayBz_1 = cupy.array(self.arrayBz_1)
        self.arrayBz_2 = cupy.array(self.arrayBz_2)
        self.arrayJ_12 = cupy.array(self.arrayJ_12)
        self.arrayT = cupy.array(self.arrayT)
        self.elementos_iter = cupy.array(self.elementos_iter)
        ##print("elementos:\n", self.elementos_iter)
...
#-------------------------------------------------------------------------------------------------------    
    ...
    def hamiltonianaNova(self, parametros):
        ...
        #input()
        return 0.5*(cupy.multiply(j_1, self.tensorProductSigZIdent, dtype=cupy.complex) + cupy.multiply(j_2,self.tensorProductIdentSigZ, dtype=cupy.complex) + 0.5*cupy.multiply(j_12,(self.tensorProductSigZSigZ - self.tensorProductSigZIdent - self.tensorProductIdentSigZ), dtype=cupy.complex) + cupy.multiply(bz_1,self.tensorProductSigXIdent, dtype=cupy.complex) + cupy.multiply(bz_2,self.tensorProductIdentSigX, dtype=cupy.complex))

    
    #Definindo a função operador temporal.
    def u(self,t, h):
        eq1 = cupy.multiply(h,t, dtype=cupy.complex)
        eq2 = cupy.multiply(eq1,(1j),dtype=cupy.complex)
        eq3 = cupy.multiply(-1, eq2, dtype=cupy.complex)
        eq3 = cupy.array(eq3, dtype=cupy.float32)
        eq3 = cupy.reshape(eq3, (4,4))
        eq3 = eq3.toDlpack()
        
        eq3 =tf.reshape(tf.experimental.dlpack.from_dlpack(eq3), [4,4])
        #print(eq3)
        #result = expm((np.matmul(np.matmul(h,t),(-1j))))
        result = tf.linalg.expm(tf.reshape(eq3,[4,-1]))
        result = tf.experimental.dlpack.to_dlpack(result)
        result = cupy.fromDlpack(result)
        ##print('result:', result)
        return result
    ...
    def ro(self,t, h):
        ...
        ##print("matrix dagger:\n", np.matrix(u).getH())
        #input()
        return cupy.dot(cupy.dot(u,self.ro0), cupy.transpose(cupy.conjugate(u)))
    ...
    def Ox1(self,ro):
     #   print("\na:{}/nb:{}".format(self.tensorProductSigXIdent, ro))
        a = cupy.dot(self.tensorProductSigXIdent, ro)
        return cupy.trace(a)


    #Definindo a função O^(2)_x 
    def Ox2(self,ro):
        a = cupy.dot(self.tensorProductIdentSigX, ro)
        return cupy.trace(a)

    #--------------------------------------------------
    #Definindo a função O^(1)_y 
    def Oy1(self,ro):
        a = cupy.dot(self.tensorProductSigYIdent, ro)
        return cupy.trace(a)


    #Definindo a função O^(2)_y 
    def Oy2(self,ro):
        a = cupy.dot(self.tensorProductIdentSigY, ro)
        return cupy.trace(a)

    #--------------------------------------------------
    #Definindo a função O^(1)_z 
    def Oz1(self,ro):
        a = cupy.dot(self.tensorProductSigZIdent, ro)
        return cupy.trace(a)


    #Definindo a função O^(2)_z 
    def Oz2(self,ro):
        a = cupy.dot(self.tensorProductIdentSigZ, ro)
        return cupy.trace(a)
    ...
    def calc_obs(self, hvalor):
        resultsOx = cupy.empty(shape=len(self.arrayT)*6)
        for idx, t in enumerate(self.arrayT):
            rovalor = self.ro(t,hvalor)
            resultsOx[idx*6] = self.Ox1(rovalor)
            resultsOx[idx*6+1] = self.Ox2(rovalor)
            resultsOx[idx*6+2] = self.Oy1(rovalor)
            resultsOx[idx*6+3] = self.Oy2(rovalor)
            resultsOx[idx*6+4] = self.Oz1(rovalor)
            resultsOx[idx*6+5] = self.Oz2(rovalor)
        return resultsOx
    ...
    def criaFrameNovo(self):
        ...
        t1 = t0 = perf_counter()
        arr_cu = cupy.array(self.elementos_iter)
        #x = da.from_array(arr_cu, chunks=(1,5))
        client = Client(threads_per_worker=300, n_workers=4, resources= {'GPU': 0})
        #cl_sc = client.scatter(arr_cu)
        s1 = client.scatter(arr_cu)
        #print("s1:\n",s1.vizualize())
        s3 = s1.result()
       # print("len:\n", len(s3))
       # print("s3", s3[:5])
        reslts_hvalor = client.map(self.hamiltonianaNova, s3)
        #print("s2:\n", reslts_hvalor)
        #wait(reslts_hvalor)
        resultsOxJ = client.map(self.calc_obs, reslts_hvalor)
        wait(resultsOxJ)
        t1 = perf_counter()
        colunas = int(((((self.tFinal - self.tInicial)/self.passoT)+1)*6))
        linhas = int(len(self.elementos_iter.get()))
        ...
        ##print('linhas:', linhas)
        aux1 = []
        for future in as_completed(resultsOxJ):
            aux1.append(future.result().get())
        #print(resultsOxJ[:2])
        #print(type(resultsOxJ))
        #aux1 = resultsOxJ.result().get()
        #Compila o resultado com os elementos referentes a cada resultado
        result = np.hstack((np.reshape(np.array(self.elementos_iter.get()), (linhas, 5)),np.reshape(np.array(aux1), (linhas, colunas))))
        #print(result.shape)
        return result
